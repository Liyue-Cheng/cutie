...
mod tests_db_config {
    ...
    fn test_database_config_default() {
        ...
        assert_eq!(config.filename, "cutie.db");
    ...
    fn test_connection_string() {
        ...
        let conn_str = config.connection_string(&db_path);

        assert!(conn_str.starts_with("sqlite:///tmp/test.db?"));
        assert!(conn_str.contains("journal_mode=WAL"));
        assert!(conn_str.contains("foreign_keys=ON"));
        assert!(conn_str.contains("synchronous=NORMAL"));
        assert!(conn_str.contains("cache_size="));
    }

    #[test]
    fn test_environment_specific_configs() {
        let dev_config = DatabaseConfig::development();
        assert!(dev_config.query_logging);
        assert_eq!(dev_config.slow_query_threshold_ms, 100);
        assert!(dev_config.auto_migrate);

        let prod_config = DatabaseConfig::production();
        assert_eq!(prod_config.max_connections, 20);
        assert!(!prod_config.query_logging);
        assert_eq!(prod_config.synchronous, SynchronousMode::Full);
        assert!(!prod_config.auto_migrate);

        let test_config = DatabaseConfig::test();
        assert_eq!(test_config.filename, ":memory:");
        assert_eq!(test_config.max_connections, 1);
        assert!(test_config.auto_migrate);
    }
    ...
    fn test_database_config_validation() {
        ...
        config.filename = "test.db".to_string();
        ...
        assert!(config.validate().is_err());
        config.connect_timeout_seconds = 30;

        // 无效缓存大小
        config.cache_size_kb = -1;
        assert!(config.validate().is_err());
    }

    #[test]
    fn test_synchronous_mode_pragma() {
        assert_eq!(SynchronousMode::Full.as_pragma_value(), "FULL");
        assert_eq!(SynchronousMode::Normal.as_pragma_value(), "NORMAL");
        assert_eq!(SynchronousMode::Off.as_pragma_value(), "OFF");
    }
}
