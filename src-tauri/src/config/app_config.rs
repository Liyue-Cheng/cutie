...
impl AppConfig {
    ...
    pub fn from_env() -> Result<Self, AppError> {
        ...
        if let Ok(timeout_str) = env::var("CUTIE_REQUEST_TIMEOUT") {
            config.request_timeout_seconds = timeout_str
                .parse()
    ...
    pub fn from_file(config_path: &PathBuf) -> Result<Self, AppError> {
        let config_content = std::fs::read_to_string(config_path).map_err(|e| {
            AppError::configuration_error(format!("Failed to read config file: {}", e))
        })?;

        let config: AppConfig = toml::from_str(&config_content).map_err(|e| {
            AppError::configuration_error(format!("Failed to parse config file: {}", e))
        })?;
    ...
    pub fn save_to_file(&self, config_path: &PathBuf) -> Result<(), AppError> {
        let config_content = toml::to_string_pretty(self).map_err(|e| {
            AppError::configuration_error(format!("Failed to serialize config: {}", e))
        })?;

        // 确保目录存在
        if let Some(parent) = config_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                AppError::configuration_error(format!("Failed to create config directory: {}", e))
            })?;
        }

        std::fs::write(config_path, config_content).map_err(|e| {
            AppError::configuration_error(format!("Failed to write config file: {}", e))
        })?;
    ...
    pub fn validate(&self) -> Result<(), AppError> {
        ...
        if !self.data_dir.exists() {
            std::fs::create_dir_all(&self.data_dir).map_err(|e| {
                AppError::configuration_error(format!("Failed to create data directory: {}", e))
            })?;
        }
        ...
        if self.ai_enabled {
            if let Some(ref ai_config) = self.ai_config {
                ...
                return Err(AppError::configuration_error(
                    "AI is enabled but no AI config provided",
                ));
            }
        }
...
impl Environment {
    ...
    pub fn from_str(s: &str) -> Result<Self, AppError> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(AppError::configuration_error(format!(
                "Invalid environment: {}",
                s
            ))),
        }
...
mod tests {
    ...
    fn test_log_level_string() {
        ...
        assert_eq!(config.log_level_string(), "info");

        config.log_level = LogLevel::Warn;
        assert_eq!(config.log_level_string(), "warn");
    }
    ...
    fn test_config_paths() {
        ...
        assert!(db_path
            .to_string_lossy()
            .contains(&config.database.filename));
