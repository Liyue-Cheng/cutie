...
impl TomlSettingRepository {
    ...
    async fn load_file(&self) -> Result<TomlSettingsFile, DbError> {
        ...
        // 解析TOML
        let settings: TomlSettingsFile = toml::from_str(&content)
            .map_err(|e| DbError::ConnectionError(
                sqlx::Error::Configuration(format!("Invalid TOML format: {}", e).into())
            ))?;
    ...
    async fn save_file(&self, settings: &TomlSettingsFile) -> Result<(), DbError> {
        ...
        let content = toml::to_string_pretty(settings)
            .map_err(|e| DbError::ConnectionError(
                sqlx::Error::Configuration(format!("Failed to serialize TOML: {}", e).into())
            ))?;
    ...
    fn toml_setting_to_setting(key: String, toml_setting: &TomlSetting) -> Result<Setting, DbError> {
        let created_at = DateTime::parse_from_rfc3339(&toml_setting.created_at)
            .map_err(|_| DbError::ConnectionError(
                sqlx::Error::Configuration("Invalid created_at format".into())
            ))?
            .with_timezone(&Utc);
        
        let updated_at = DateTime::parse_from_rfc3339(&toml_setting.updated_at)
            .map_err(|_| DbError::ConnectionError(
                sqlx::Error::Configuration("Invalid updated_at format".into())
            ))?
            .with_timezone(&Utc);
    ...
    async fn upsert_setting(&self, setting: &Setting) -> Result<Setting, DbError> {
        ...
        let updated_setting = if settings_file.settings.contains_key(&setting.key) {
            ...
            Setting {
                ...
                value: setting.value.clone(),
                description: setting.description.clone(),
                created_at: settings_file.settings[&setting.key].created_at
        ...
        settings_file.settings.insert(setting.key.clone(), toml_setting);
...
mod tests {
    ...
    async fn test_toml_setting_repository_update() {
        ...
        // 更新值
        let updated_setting = Setting::new("test.key".to_string(), "updated_value".to_string(), now);
    ...
    async fn test_toml_setting_repository_batch_get() {
        ...
        let keys = vec!["key1".to_string(), "key3".to_string(), "nonexistent".to_string()];
    ...
    async fn test_toml_setting_repository_file_persistence() {
        ...
        // 第一个仓库实例
        {
            ...
            let setting = Setting::new("persistent.key".to_string(), "persistent_value".to_string(), now);
