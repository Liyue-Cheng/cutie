/// 更新 Project API - 单文件组件
use axum::{
    extract::{Path, State},
    response::{IntoResponse, Response},
    Json,
};
use sqlx::{Sqlite, Transaction};
use uuid::Uuid;

use crate::{
    entities::{ProjectDto, UpdateProjectRequest},
    features::shared::{repositories::ProjectRepository, TransactionHelper},
    infra::{
        core::{AppError, AppResult},
        http::error_handler::success_response,
    },
    startup::AppState,
};

// ==================== 文档层 ====================
/*
CABC for `update_project`

## 1. 端点签名 (Endpoint Signature)

PATCH /api/projects/{id}

## 2. 预期行为简介 (High-Level Behavior)

### 2.1. 用户故事 / 场景 (User Story / Scenario)

> 作为一个用户，我想要更新项目的信息，
> 包括名称、描述、状态、截止日期和关联的 Area。

### 2.2. 核心业务逻辑 (Core Business Logic)

根据项目 ID 更新项目信息。支持部分更新（PATCH），只更新提供的字段。
如果状态从 COMPLETED 改为 ACTIVE，清除 completed_at。

## 3. 输入输出规范 (Request/Response Specification)

### 3.1. 请求 (Request)

**URL Parameters:**
- `id` (UUID, required): 项目ID

**请求体 (Request Body):** `application/json`

```json
{
  "name": "string (optional, 1-200字符)",
  "description": "string | null (optional)",
  "status": "ACTIVE | COMPLETED (optional)",
  "due_date": "YYYY-MM-DD | null (optional)",
  "area_id": "uuid | null (optional)"
}
```

### 3.2. 响应 (Responses)

**200 OK:**

```json
{
  "id": "uuid",
  "name": "string",
  "description": "string | null",
  "status": "ACTIVE | COMPLETED",
  "due_date": "YYYY-MM-DD | null",
  "completed_at": "ISO8601 timestamp | null",
  "area_id": "uuid | null",
  "total_tasks": 0,
  "completed_tasks": 0,
  "created_at": "ISO8601 timestamp",
  "updated_at": "ISO8601 timestamp"
}
```

**404 Not Found:**
**422 Unprocessable Entity:**
**409 Conflict:**

## 4. 验证规则 (Validation Rules)

- `name`: 如果提供，长度在 1-200 字符之间，且不与其他项目重复
- `description`: 如果提供，长度不超过 2000 字符
- `due_date`: 如果提供，必须 >= 今天
- `status`: 如果提供，必须是 ACTIVE 或 COMPLETED

## 5. 业务逻辑详解 (Business Logic Walkthrough)

1. 验证输入。
2. 获取写入许可。
3. 开启事务。
4. 查询项目是否存在。
5. 如果提供 name，检查名称唯一性（排除当前项目）。
6. 更新项目字段。
7. 如果状态从 COMPLETED 改为 ACTIVE，清除 completed_at。
8. 如果状态从 ACTIVE 改为 COMPLETED，设置 completed_at。
9. 更新数据库。
10. 写入 Event Outbox：发送 `project.updated` 事件。
11. 提交事务。
12. 返回更新后的 ProjectDto。

## 6. 边界情况 (Edge Cases)

- **项目不存在:** 返回 `404`。
- **名称已存在:** 返回 `409`。
- **重新打开已完成的项目:** 清除 completed_at，但不改变任务的完成状态。

## 7. 预期副作用 (Expected Side Effects)

- **数据库写入:**
    - **`SELECT`:** 查询项目和检查名称唯一性。
    - **`UPDATE`:** 1条记录到 `projects` 表。
    - **`INSERT`:** 1条记录到 `event_outbox` 表。
- **SSE 事件:**
    - **事件类型:** `project.updated`
    - **载荷:** 完整的 `ProjectDto` 对象

## 8. 契约 (Contract)

- HTTP 响应与 SSE 事件载荷完全一致
- 重新打开项目时不影响已完成的任务
*/

// ==================== HTTP 处理器 ====================
pub async fn handle(
    State(app_state): State<AppState>,
    Path(project_id): Path<Uuid>,
    Json(request): Json<UpdateProjectRequest>,
) -> Response {
    match logic::execute(&app_state, project_id, request).await {
        Ok(project_dto) => success_response(project_dto).into_response(),
        Err(err) => err.into_response(),
    }
}

// ==================== 业务逻辑层 ====================
mod logic {
    use super::*;

    pub async fn execute(
        app_state: &AppState,
        project_id: Uuid,
        request: UpdateProjectRequest,
    ) -> AppResult<ProjectDto> {
        // 1. 验证
        validation::validate_update_request(&request)?;

        // 2. 获取写入许可
        let _permit = app_state.acquire_write_permit().await;

        let mut tx = TransactionHelper::begin(app_state.db_pool()).await?;

        // 3. 查询项目
        let mut project = ProjectRepository::find_by_id(&mut *tx, project_id)
            .await?
            .ok_or_else(|| AppError::not_found("Project", project_id.to_string()))?;

        // 4. 如果更新名称，检查唯一性
        if let Some(ref new_name) = request.name {
            let exists =
                ProjectRepository::check_name_exists(&mut tx, new_name, Some(project_id)).await?;
            if exists {
                return Err(AppError::conflict("项目名称已存在"));
            }
        }

        let now = app_state.clock().now_utc();

        // 5. 更新字段
        if let Some(name) = request.name {
            project.name = name;
        }
        if let Some(description) = request.description {
            project.description = description;
        }
        if let Some(status) = request.status {
            let old_status = project.status.clone();
            project.status = status.clone();

            // 处理状态变化
            use crate::entities::ProjectStatus;
            match (&old_status, &status) {
                (ProjectStatus::Completed, ProjectStatus::Active) => {
                    // 重新打开项目
                    project.completed_at = None;
                }
                (ProjectStatus::Active, ProjectStatus::Completed) => {
                    // 完成项目
                    project.completed_at = Some(now);
                }
                _ => {}
            }
        }
        if let Some(due_date) = request.due_date {
            project.due_date = due_date;
        }
        if let Some(area_id) = request.area_id {
            project.area_id = area_id;
        }

        project.updated_at = now;

        // 6. 更新数据库
        ProjectRepository::update(&mut tx, &project).await?;

        // 7. 组装 DTO
        let project_dto = ProjectDto {
            id: project.id,
            name: project.name.clone(),
            description: project.description.clone(),
            status: project.status.clone(),
            due_date: project.due_date,
            completed_at: project.completed_at,
            area_id: project.area_id,
            created_at: project.created_at,
            updated_at: project.updated_at,
        };

        // 8. 写入 Event Outbox
        events::write_project_updated_event(app_state, &mut tx, &project_dto, now).await?;

        // 9. 提交事务
        TransactionHelper::commit(tx).await?;

        Ok(project_dto)
    }
}

// ==================== 验证层 ====================
mod validation {
    use super::*;

    pub fn validate_update_request(request: &UpdateProjectRequest) -> AppResult<()> {
        // 验证 name
        if let Some(ref name) = request.name {
            let name = name.trim();
            if name.is_empty() {
                return Err(AppError::validation_error(
                    "name",
                    "名称不能为空",
                    "NAME_EMPTY",
                ));
            }
            if name.len() > 200 {
                return Err(AppError::validation_error(
                    "name",
                    "名称长度不能超过200字符",
                    "NAME_TOO_LONG",
                ));
            }
        }

        // 验证 description
        if let Some(Some(ref desc)) = request.description {
            if desc.len() > 2000 {
                return Err(AppError::validation_error(
                    "description",
                    "描述长度不能超过2000字符",
                    "DESCRIPTION_TOO_LONG",
                ));
            }
        }

        // 验证 due_date
        if let Some(Some(due_date)) = request.due_date {
            let today = chrono::Utc::now().date_naive();
            if due_date < today {
                return Err(AppError::validation_error(
                    "due_date",
                    "截止日期不能早于今天",
                    "DUE_DATE_IN_PAST",
                ));
            }
        }

        Ok(())
    }
}

// ==================== 事件层 ====================
mod events {
    use super::*;
    use crate::infra::events::{
        models::DomainEvent,
        outbox::{EventOutboxRepository, SqlxEventOutboxRepository},
    };
    use chrono::{DateTime, Utc};

    pub async fn write_project_updated_event(
        app_state: &AppState,
        tx: &mut Transaction<'_, Sqlite>,
        project_dto: &ProjectDto,
        now: DateTime<Utc>,
    ) -> AppResult<()> {
        let outbox_repo = SqlxEventOutboxRepository::new(app_state.db_pool().clone());
        let payload = serde_json::to_value(project_dto)?;

        let event = DomainEvent::new(
            "project.updated",
            "project",
            project_dto.id.to_string(),
            payload,
        )
        .with_aggregate_version(now.timestamp_millis());

        outbox_repo.append_in_tx(tx, &event).await?;

        Ok(())
    }
}
