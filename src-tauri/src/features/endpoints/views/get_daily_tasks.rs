...
mod logic {
    ...
    pub async fn execute(app_state: &AppState, date_str: &str) -> AppResult<GetDailyTasksResponse> {
        ...
        // 2. 实例化循环任务（如果有）
        use crate::features::shared::RecurrenceInstantiationService;
    ...
    pub async fn find_tasks_for_date(
        ...
        let query = r#"
            SELECT DISTINCT t.id, t.title, t.glance_note, t.detail_note, t.estimated_duration,
                   t.subtasks, t.sort_positions, t.project_id, t.section_id, t.area_id, t.due_date, t.due_date_type, t.completed_at, t.archived_at,
