...
pub async fn create_task(
    ...
    sqlx::query(
        "INSERT INTO tasks (id, project_id, title, status, due_date, completed_at, sort_key, metadata, created_at, updated_at, deleted_at, remote_updated_at)
...
pub async fn update_task(
    ...
    let mut task = sqlx::query_as::<_, Task>("SELECT * FROM tasks WHERE id = $1 FOR UPDATE")
        .bind(id)
        .fetch_one(&mut *tx)
        .await
        .map_err(|e| e.to_string())?;
    ...
    sqlx::query(
        "UPDATE tasks SET project_id = $1, title = $2, status = $3, due_date = $4, completed_at = $5, sort_key = $6, metadata = $7, updated_at = $8 WHERE id = $9",
    )
    .bind(&task.project_id)
    .bind(&task.title)
    .bind(&task.status)
...
pub async fn list_inbox_tasks(pool: State<'_, DbPool>) -> Result<Vec<Task>, String> {
    let tasks = sqlx::query_as::<_, Task>(
        "SELECT t.*
         FROM tasks t
         LEFT JOIN task_activity_links tal ON t.id = tal.task_id
