    ...
    <div v-if="show" class="modal-overlay" @click="handleOverlayClick">
      <div class="modal-dialog" @click.stop>
        <div class="dialog-header">
          <h3>{{ $t('project.action.editSection') }}</h3>
        ...
        <div class="dialog-body">
          <div class="form-group">
            <label for="section-title"
              >{{ $t('project.field.sectionTitle') }}
              <span class="required">{{ $t('common.label.required') }}</span></label
            >
            <input
              id="section-title"
              ref="titleInputRef"
        ...
        <div class="dialog-footer">
          ...
          <div class="footer-right">
            ...
            <button
              ...
              @click="handleSubmit"
            >
              {{ isSubmitting ? $t('common.button.saving') : $t('common.action.save') }}
            </button>
          </div>
        </div>
      </div>
...
// 当对话框显示时，加载章节数据并聚焦到标题输入框
watch(
  () => props.show,
  async (newShow) => {
    if (newShow && props.sectionId) {
      // 加载章节数据
      const section = projectStore.getSectionById(props.sectionId)
      if (section) {
        formData.value = {
          title: section.title,
...
async function handleSubmit() {
  ...
  if (!title || isSubmitting.value || !props.sectionId) return

  isSubmitting.value = true

  try {
    logger.info(LogTags.UI, '更新章节', formData.value)
...
async function handleDelete() {
  ...
  try {
    logger.info(LogTags.UI, '删除章节', { sectionId: props.sectionId })
