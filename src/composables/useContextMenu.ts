...
const manager = {
  ...
  show(component: Component, props: Record<string, any> = {}, event?: MouseEvent) {
    ...
    const hideOnClickOutside = (event: Event) => {
      ...
      // 🎯 检查点击是否在对话框内部（dialog-overlay 或其子元素）
      // 如果是，允许对话框正常处理点击事件，同时关闭菜单
      const dialogOverlay = (target as Element).closest?.('.dialog-overlay')
      if (dialogOverlay) {
        // 点击在对话框内，允许事件正常传播，但仍关闭菜单
        manager.hide()
        return
      }

      // 🎯 点击在菜单和对话框外部：阻止事件传播，只关闭菜单
